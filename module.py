...
def discriminator(image, options, reuse=False, name="discriminator",update_collection=None):
    ...
    with tf.variable_scope(name):
        ...
        h0 = lrelu(ops.conv2d(image, options.df_dim, 4,4,2,2,spectral_normed=True, update_collection=update_collection, stddev=0.02,name='d_h0_conv'))
        # h0 is (128 x 128 x self.df_dim)
        h1 = lrelu(instance_norm(ops.conv2d(h0, options.df_dim*2, 4,4,2,2,spectral_normed=True, update_collection=update_collection, stddev=0.02, name='d_h1_conv'), 'd_bn1'))
        # h1 is (64 x 64 x self.df_dim*2)
        h2 = lrelu(instance_norm(ops.conv2d(h1, options.df_dim*4, 4,4,2,2,spectral_normed=True, update_collection=update_collection, stddev=0.02, name='d_h2_conv'), 'd_bn2'))
        # h2 is (32x 32 x self.df_dim*4)
        h3 = lrelu(instance_norm(ops.conv2d(h2, options.df_dim*8, 4,4,1,1,spectral_normed=True, update_collection=update_collection, stddev=0.02, name='d_h3_conv'), 'd_bn3'))
        # h3 is (32 x 32 x self.df_dim*8)
        h4 = ops.conv2d(h3, 1, 4,4,1,1,spectral_normed=True, update_collection=update_collection, stddev=0.02, name='d_h3_pred')
        # h4 is (32 x 32 x 1)
        return h4
